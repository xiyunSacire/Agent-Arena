...
# --- FastAPI 应用实例化 ---
app = FastAPI(
    title="HBPU Agent Arena Server",  # 修改这里：API 的标题
    description="智能体竞技场服务端",  # 修改这里：API 的描述（支持 Markdown）
    version="1.4.0",  # 修改这里：API 的版本
    lifespan=lifespan
) 
# API 标题，会显示在 /docs 页面 lifespan=lifespan绑定生命周期管理函数
